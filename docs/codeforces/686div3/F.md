# codeforces/686div3/F

## Code
main.cpp
{% raw %}
```cpp
// codeforces/686div3/F/main.cpp
// author: @___Johniel
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    SegTree<int> mx(n, 0, [] (auto x, auto y) { return max(x, y); });
    SegTree<int> mn(n, 1 << 30, [] (auto x, auto y) { return min(x, y); });
    ...
    for (int i = 1; i + 1 < a.size(); ++i) {
      ...
      while (small + 1 < large) {
        ...
        if (x < mn(i, mid) || x < mx(mid, n)) small = mid;
        else large = mid;
      }
      if (x == mn(i, small) && x == mx(small, n)) {
        cout << "YES"<< endl;
        cout << i << ' ' << small - i << ' ' << n - small << endl;
        found = true;
        break;
      }
      if (x == mn(i, large) && x == mx(large, n)) {
        cout << "YES"<< endl;
        cout << i << ' ' << large - i << ' ' << n - large << endl;
        found = true;
        break;
      }
    }
...
+ [repository](https://github.com/Johniel/contests/tree/master/codeforces/686div3/F)
