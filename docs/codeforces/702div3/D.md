# codeforces/702div3/D

## Code
main.cpp
{% raw %}
```cpp
// codeforces/702div3/D/main.cpp
// author: @___Johniel
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    SegTree<pair<int, int>> rmq(n, make_pair(-1, -1), [] (pair<int, int> a, pair<int, int> b) { return max(a, b); });
    ...
    while (q.size()) {
      ...
      pair<int, int> x = rmq(r.first, r.second);
      v[x.second] = d;
      if (r.first + 1 == r.second) continue;
      q.push(make_pair(d + 1, make_pair(r.first, x.second)));
      q.push(make_pair(d + 1, make_pair(x.second + 1, r.second)));
    }
...
+ [repository](https://github.com/Johniel/contests/tree/master/codeforces/702div3/D)
