...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    SegTree<pair<int, int>> rmq(a.size(), make_pair(-1, -1), [] (auto x, auto y) { return max(x, y); });
    ...
    for (int i = 0; i < n && 0 <= x; ++i) {
      ...
      pair<int, int> p = rmq(0, x+1);
      for (int j = p.second; j < x; ++j) {
        w.push_back(j);
        auto x = rmq[j];
        auto y = rmq[j + 1];
        rmq.update(j + 1, x);
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/arc110/C)
