    ...
    for (int i = 0; i < a.size(); ++i) {
      {
        int z = i;
        if (k - 1 <= z) {
          lli w = mod_comb(z, k - 1);
          lli b = (((a[i] * w) % mod) * 1) % mod;
          x = (x + b) % mod;
        }
      }
      {
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/abc151/E)
