...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    for (int i = 0; i <= n; ++i) {
      each (j, v) {
        int a = max(0, i - (j.first - 1));
        int b = max(0, i - j.second);
        bit.add(i, bit(a));
        bit.add(i, -bit(b));
      }
    }
    ...
    // for (int i = 0; i <= n; ++i) {
    //   cout << make_pair(i, bit(i, i+1)) << endl;
    // }
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/abc179/D)
