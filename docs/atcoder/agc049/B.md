...
int main(int argc, char *argv[])
{
  ...
  str s, t;
  while (cin >> n >> s >> t) {
    SegTree<pair<char, int>> rmq(n, make_pair(0, 0), [] (auto a, auto b) { return max(a, b); });
    for (int i = 0; i < s.size(); ++i) {
      rmq.update(i, make_pair(s[i], -i));
    }

    lli x = 0;
    for (int i = 0; i + 1 < s.size(); ++i) {
      if (s[i] != t[i]) {
        auto p = rmq.query(i + 1, n);
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/agc049/B)
