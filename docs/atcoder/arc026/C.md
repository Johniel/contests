...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> len) {
    ...
    for (int i = 0; i < n; ++i) {
      int begin, end, c;
      cin >> begin >> end >> c;
      v[end].push_back(make_pair(begin, c));
    }

    const lli inf = 1LL << 60;
    SegTree<lli> rmq(N, inf, [] (auto a, auto b) { return min(a, b); });
    rmq.update(0, 0);
    for (int i = 1; i < N-3; ++i) {
      lli mn = inf;
      each (j, v[i]) {
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/arc026/C)
