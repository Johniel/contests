...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    SegTree<int> rmq(N, -inf, [] (auto a, auto b) { return max(a, b); });
    for (int i = 0; i < n; ++i) {
      rmq.update(a[i], 0);
    }

    for (int i = 0; i < a.size(); ++i) {
      int begin = max(0, a[i] - k);
      int end = min(N, a[i] + k + 1);

      int mx= rmq(begin, end);
      rmq.update(a[i], mx + 1);
    }
    cout << rmq(0, N) << endl;
  }
...
+ [repository](https://github.com/Johniel/contests/tree/master/atcoder/abl/D)
