...
// codeforces/1046div2/C/main.cpp
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    const int inf = -(1 << 28);
    SegTree<int> seg(n, inf, [] (auto a, auto b) { return max(a, b); });

    for (int i = 0; i < a.size(); ++i) {
      int prev = max(seg.query(0, i), 0);
      int j = lower_bound(m[a[i]].begin(), m[a[i]].end(), i) - m[a[i]].begin();
      if (j + a[i] - 1 < m[a[i]].size()) {
        seg.set(m[a[i]].at(j + a[i] - 1), prev + a[i]);
      }
    }
    cout << max(0, seg.all_prod()) << endl;
  }
  return 0;
}
