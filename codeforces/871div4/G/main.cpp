...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; k < N; ++i) {
    v.push_back(vec<lli>());
    for (int j = 0; j <= i; ++j) {
      m[k] = make_pair(i, v[i].size());
      v[i].push_back(k * k);
      ++k;
    }
  }
  vec<PrefixSum<lli>> sum;
  for (int i = 0; i < v.size(); ++i) {
    sum.push_back(PrefixSum<lli>(v[i]));
  }

  int n;
  while (cin >> n) {
    int h = m[n].first;
    int begin = m[n].second;
    int end = m[n].second + 1;
    lli z = 0;
    while (0 <= h) {
      z += sum[h](max(0, begin), min<int>(end, sum[h].size()));
      --h;
      --begin;
    }
    cout << z << endl;
