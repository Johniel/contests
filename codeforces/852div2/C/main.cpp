...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    int end = a.size();
    while (begin < end) {
      if (mn.query(begin, end) == a[begin] || mx.query(begin, end) == a[begin]) {
        ++begin;
        continue;
      }
      if (mn.query(begin, end) == a[end - 1] || mx.query(begin, end) == a[end - 1]) {
        --end;
        continue;
      }
