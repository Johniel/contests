...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    cin >> a >> q;
    const int inf = 1 << 29;
    SegTree<pair<int, int>> mn(a.size(), make_pair(+inf, -1), [] (auto a, auto b) { return min(a, b); });
    SegTree<pair<int, int>> mx(a.size(), make_pair(-inf, -1), [] (auto a, auto b) { return max(a, b); });
    for (int i = 0; i < a.size(); ++i) {
      mn.set(i, make_pair(a[i], i));
      mx.set(i, make_pair(a[i], i));
    }
    while (q--) {
      int begin, end;
      cin >> begin >> end;
