...
int main(int argc, char *argv[])
{
  ...
  int n, q;
  while (cin >> n >> q) {
    SegTree<int> a(n, 1 << 29, [] (int x, int y) { return min(x, y); });
    SegTree<int> b(n, 1 << 29, [] (int x, int y) { return min(x, y); });
    for (int i = 0; i < n; ++i) {
      a.update(i, 0);
      b.update(i, 0);
    }
    for (int k = 0; k < q; ++k) {
      ...
      if (t == 3) {
        ...
        --x2;
        --y2;
        cout << (a(x1, x2 + 1) || b(y1, y2 + 1) ? "Yes" : "No") << endl;
      }
    }
  }
