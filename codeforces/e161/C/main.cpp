...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    for (int i = a.size() - 1; 0 < i; --i) {
      if (u.empty()) u.push_back(1);
      else {
        lli x = abs(a[i] - a[i + 1]);
        lli y = abs(a[i] - a[i - 1]);
        if (x > y) u.push_back(1);
        else u.push_back(y);
      }
    }
    reverse(u.begin(), u.end());

    PrefixSum<lli> x(v), y(u);
    ...
    while (q--) {
      ...
      --s;
      --t;
      if (s < t) cout << x(s, t) << endl;
      else cout << y(t, s) << endl;
    }
  }
  return 0;
}
