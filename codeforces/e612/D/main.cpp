...
vec<int> fn(const vec<lli>& a)
{
  PrefixSum<lli> sum(a);

  vec<int> same;
  for (int i = 0; i + 1 < a.size(); ++i) {
    same.push_back(a[i] == a[i + 1]);
  }
  PrefixSum<int> s(same);

  auto f = [&] (int i, int mid) {
    return a[i] < sum(i + 1, mid) && s(i + 1, mid - 1) != (mid - 1) - (i + 1);
  };

  vec<int> v(a.size(), inf);
  for (int i = 0; i + 1 < a.size(); ++i) {
    if (a[i] < a[i + 1]) {
      v[i] = 1;
      continue;
    ...
    while (small + 1 < large) {
      ...
      if (f(i, mid)) large = mid;
      else small = mid;
    }
    if (f(i, large)) setmin(v[i], large - i - 1);
  }
