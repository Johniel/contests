...
// codeforces/1034div3/E/main.cpp
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    BIT<int> bit(a.size() + 3);
    each (i, a) if (i <= n) bit.add(i, 1);

    BIT<int> pref(a.size() + 3);
    {
      vec<int> b = a;
      sort(b.begin(), b.end());
      b.erase(unique(b.begin(), b.end()), b.end());
      each (i, b) if (i <= n) pref.add(i, 1);
    }

    // for (int mex = 0; mex <= a.size(); ++mex) cout << mex << ' ' << pref(0, mex) << endl;

    vec<pair<int, int>> v;
    const int inf = 1 << 28;
    for (int mex = 0; mex <= a.size(); ++mex) {
      if (pref(0, mex) == mex) {
        int mn = freq[mex];
        int mx = n - mex;
    ...
    BIT<int> res(a.size() + 3);
    each (k, v) {
      auto [mn, mx] = k;
      if (mn == inf || mx == inf) continue;
      res.add(mn, +1);
      res.add(mx + 1, -1);
    }
