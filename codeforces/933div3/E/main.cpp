...
struct SegTree {
  ...
  const F fn;
  const T e;
  SegTree(size_t n_, T e_, F fn_) : e(e_), origin_size(n_), fn(fn_), n(my_bit_ceil(n_)) {
    assert(fn(e, e) == e);
    v.resize(2 * n - 1, e);
  }
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> h >> w >> k >> d) {
    ...
    vec<lli> v;
    for (int i = 0; i < h; ++i) {
      SegTree<lli> seg(w, inf, [] (auto a, auto b) { return min(a, b); });
      seg.set(0, 1);
      for (int j = 1; j < w; ++j) {
        seg.set(j, seg.query(max(j - d - 1, 0), j) + g[i][j] + 1);
      }
      v.push_back(seg.get(w - 1));
    }
    lli z = inf;
