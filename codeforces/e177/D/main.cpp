...
int main(int argc, char *argv[])
{
  ...
  while (cin >> v) {
    const int C = 26 + 3;
    static lli dp[C][N];
    const int L = accumulate(v.begin(), v.end(), 0);
    for (int i = 0; i < C; ++i) {
      for (int j = 0; j <= L; ++j) {
        dp[i][j] = 0;
      }
    ...
    for (int i = 0; i < v.size(); ++i) {
      for (int j = 0; j <= L; ++j) {
        (dp[i + 1][j] += dp[i][j]) %= mod;
        if (v[i]) (dp[i + 1][j + v[i]] += dp[i][j]) %= mod;
      }
    }
    lli z = dp[v.size()][L / 2];
    (z *= math::fact[L / 2]) %= mod;
    (z *= math::fact[(L + 1) / 2]) %= mod;
    for (int i = 0; i < v.size(); ++i) {
      (z *= math::mod_inverse(math::fact[v[i]], mod)) %= mod;
    }
    cout << z << endl;
  }
  return 0;
