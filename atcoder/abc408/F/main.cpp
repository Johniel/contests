...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> d >> r) {
    ...
    sort(v.begin(), v.end());
    const int inf = (1 << 28);
    SegTree<int> seg(n, -inf, [] (int a, int b) { return max(a, b); });
    vec<int> u(n + 1, -inf);
    int curr = 0;
    for (int k = 0; k < v.size(); ++k) {
      auto [h, idx] = v[k];
      while (curr < k && v[curr].first <= h - d) {
        seg.set(v[curr].second, u[v[curr].second]);
        ++curr;
      }
      int begin = max(0, idx - r);
      int end = min<int>(seg.size(), idx + r + 1);
