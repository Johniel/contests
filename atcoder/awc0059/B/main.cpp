...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m >> k) {
    ...
    for (int i = 0; i < s.size(); ++i) {
      ...
      if (f) v.push_back(make_pair(i, sum));
    }
    sort(v.begin(), v.end());
    if (v.size() <= k) {
    } else {
      ...
      vec<pair<int, lli>> tmp;
      each (i, v) {
        if (u[k - 1] <= i.second) tmp.push_back(i);
      }
      v = tmp;
    }
    each (i, v) cout << i.first +1 << endl;
  }
  return 0;
}
