...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    vec<pair<int, pair<int, int>>> es;
    for (int w = A; 1 < w; --w) {
      vec<int> v;
      for (int mul = 1; w * mul < A; ++mul) {
        if (inv[w * mul] != -1) {
          if (v.empty()) {
            v.push_back(inv[w * mul]);
          } else {
            if (!uf.same(v.front(), inv[w * mul]) && !uf.same(v.back(), inv[w * mul])) {
              v.push_back(inv[w * mul]);
            }
          }
        }
      ...
      for (int i = 0; i < v.size(); ++i) {
        for (int j = 0; j < i; ++j) {
          if (!uf.same(v[i], v[j]) && __gcd(a[v[i]], a[v[j]]) == w) {
            uf.unite(v[i], v[j]);
            z += w;
          }
        }
