...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m>> s) {
    ...
    lli z = 1;
    each (k, t) {
      z *= math::fact[k.second.size()];
      z %= mod;
      // cout << k.second << endl;
      for (char c = 'a'; c <= 'z'; ++c) {
        int cnt = count(k.second.begin(), k.second.end(), c);
        if (cnt <= 1) continue;
        z *= math::mod_inverse(math::fact[cnt], mod);
        z %= mod;
        // z *= math::mod_inverse(2, mod);
        // z %= mod;
        f = false;
      }
    }
    if (f) (z *= math::mod_inverse(2, mod)) %= mod;
    cout << z << endl;
    // cout << z*math::mod_inverse(2, mod) << endl;
  }
  return 0;
}
