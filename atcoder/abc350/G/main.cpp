...
int main(int argc, char *argv[])
{
  ...
  while (scanf("%d %d\n", &n, &q) == 2) {
    ...
    for (int i = 0; i < q; ++i) {
      int a, b, c;
      scanf("%d %d %d\n", &a, &b, &c);
      lli A = 1 + ((a * (1LL + x)) % mod) % 2;
      lli B = 0 + ((b * (1LL + x)) % mod) % n;
      lli C = 0 + ((c * (1LL + x)) % mod) % n;
      // cout << A << ' ' << B+1 << ' ' << C+1 << endl;
      if (A == 1) {
        adj[B].insert(C);
        adj[C].insert(B);
      ...
      if (A == 2) {
        int y = -1;
        if (uf.same(B, C) && (uf.diff(B, C) == 2 || uf.diff(B, C) == 0)) {
          unless (adj[B].size() < adj[C].size()) swap(B, C);

          pair<int, int> key = make_pair(B, C);
          if (memo.count(key)) {
            y = memo[key];
          } else {
            auto itr = adj[B].lower_bound(*adj[C].begin());
            while (itr != adj[B].end() && *itr <= *adj[C].rbegin()) {
              if (adj[C].count(*itr)) {
                y = *itr;;
                break;
              }
