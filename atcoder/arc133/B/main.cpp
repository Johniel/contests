...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    auto better = [] (pair<int, int> x, pair<int, int> y) {
      if (x.first != y.first) return x.first < y.first ? y : x;
      return x.second < y.second ? x : y;
    };

    // {len,idx}
    SegTree<pair<int, int>> seg(a.size()+1, make_pair(0, -9999), better);
    for (int i = 0; i < a.size(); ++i) {
      vec<pair<int, int>> v;
      for (int bi = a[i]; bi <= n; bi += a[i]) {
        v.push_back(make_pair(invb[bi], bi));
      }
      sort(v.begin(), v.end());
      reverse(v.begin(), v.end());
      each (j, v) {
        auto [_, bi] = j;
        auto prev = seg(0, invb[bi]);
        auto curr = prev;
        ++curr.first;
        curr.second = invb[bi];
        seg.set(invb[bi], better(curr, seg.get(invb[bi])));
      }
    }
