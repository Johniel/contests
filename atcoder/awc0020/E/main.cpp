...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    cin >> c;
    int z = 0;
    SegTree<lli> seg(c, -(1LL << 60), [] (auto a, auto b) { return max(a, b); });
    for (int i = 0; i < w.size(); ++i) {
      int small = 0;
      int large = seg.size();
      while (small + 1 < large) {
        int mid = (small + large) / 2;
        if (w[i] <= seg.query(0, mid)) large = mid;
        else small = mid;
      }
      if (0) {
      } else if (w[i] <= seg.query(0, small)) {
        seg.set(small - 1, -(1LL << 60));
        ++z;
      } else if (w[i] <= seg.query(0, large)) {
        seg.set(large - 1, -(1LL << 60));
        ++z;
      }
    }
    cout << z << endl;
