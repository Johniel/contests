...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> q) {
    ...
    SegTree<lli> mn(a, +inf, [] (lli x, lli y) { return min(x, y); });
    SegTree<lli> mx(a, -inf, [] (lli x, lli y) { return max(x, y); });
    while (q--) {
      int begin, end;
      cin >> begin >> end;
      --begin;
      cout << mx.query(begin, end) - mn.query(begin, end) << endl;
    }
  }
  return 0;
}
