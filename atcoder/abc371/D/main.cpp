...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    int q;
    cin >> q;
    vec<pair<lli, lli>> qs(q);
    cin >> qs;

    vec<lli> v = p;
    each (k, qs) v.push_back(k.first), v.push_back(k.second);
    sort(v.begin(), v.end());
    ...
    SegTree<lli> seg(m.size() + 1, 0, [] (auto x, auto y) { return x + y; });
    for (int i = 0; i < p.size(); ++i) {
      seg.set(p[i], x[i]);
    }
    each (k, qs) {
      cout << seg.query(k.first, k.second + 1) << endl;
    }
  }
  return 0;
}
