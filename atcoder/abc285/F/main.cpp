...
int main(int argc, char *argv[])
{
  ...
  int q;
  while (cin >> n >> s >> q) {
    SegTree<bool> seg(n, true, [&] (auto a, auto b) { return a && b; });

    using A = array<int, 30>;
    A E;
    E.fill(0);

    SegTree<A> occ(n, E, [] (A a, A b) {
      A x;
      for (int i = 0; i < x.size(); ++i) x[i] = a[i] + b[i];
      return x;
    });

    for (int i = 0; i < s.size(); ++i) {
      if (i + 1 < s.size()) seg.update(i, s[i] <= s[i + 1]);
      A a;
      a.fill(0);
      ++a[s[i] - 'a'];
      occ.update(i, a);
    ...
    while (q--) {
      ...
      if (op == 2) {
        ...
        bool f = seg.query(begin, end - 1);

        A x = occ.query(begin, end);
        int a, b;
        a = b = -1;
