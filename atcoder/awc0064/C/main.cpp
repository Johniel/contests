...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    for (int k = 0; k < n; ++k) {
      ...
      for (int x = 0; x + v[k].second <= m; ++x) {
        setmax(dp[j][x + v[k].second], dp[i][x] + v[k].first);
      }
    }
    cout << *max_element(&dp[0][0], &dp[2 - 1][M - 1]) << endl;
  }
  return 0;
}
