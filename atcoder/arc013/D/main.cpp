  ...
  while (cin >> n) {
    ...
    for (int i = 0; i < v.size(); ++i) {
      for (int j = 0; j < v.size(); ++j) {
        ...
        if (x * x + y * y < d[i]) g[i].push_back(j);
      }
    }

    vec<vec<int>> color = detect_scc(n);
    map<int, int> rename;
    for (int i = 0; i < color.size(); ++i) {
      each (j, color[i]) rename[j] = i;
    }

    vec<int> deg(color.size(), 0);
    for (int i = 0; i < n; ++i) {
      each (j, g[i]) {
        int src = rename[i];
        int dst = rename[j];
    ...
    cout << count(deg.begin(), deg.end(), 0) << endl;
  }
  return 0;
}
