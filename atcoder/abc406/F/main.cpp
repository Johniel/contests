...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    vec<pair<int, int>> v = et.build();
    SegTree<lli> seg(n + n, 0, [] (lli a, lli b) { return a + b; });
    ...
    while (q--) {
      ...
      if (op == 1) {
        ...
        cin >> x >> w;
        --x;
        seg.set(v[x].first, seg.get(v[x].first) + w);
      }
      if (op == 2) {
        int e;
        cin >> e;
        ...
        int a = es[e].first;
        int b = es[e].second;
        int x = (v[a].first < v[b].first ? b : a);
        lli s = seg.query();
        lli t = seg.query(v[x].first, v[x].second);
        s -= t;
        cout << abs(s - t) << endl;
      }
    }
