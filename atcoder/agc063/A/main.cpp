...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> s) {
    const int M = s.size();
    SegTree<pair<int, int>> seg(M, make_pair(1, n - 1), [] (auto a, auto b) { return min(a, b); });
    SegTree<pair<int, int>> segA(M, make_pair(1, n - 1), [] (auto a, auto b) { return min(a, b); });
    SegTree<pair<int, int>> segB(M, make_pair(1, n - 1), [] (auto a, auto b) { return min(a, b); });
    for (int i = 0; i < M; ++i) {
      if (s[i] == 'A') {
        segA.update(i, make_pair(0, i));
        segB.update(i, make_pair(1, i));
      }
      if (s[i] == 'B') {
        segA.update(i, make_pair(1, i));
        segB.update(i, make_pair(0, i));
      }
    ...
    for (int k = 0; k < n; ++k) {
      if (k % 2) {
        pair<int, int> p = segA.query(0, M);
        if (p.first == 0) {
          segA.update(p.second, make_pair(1, p.second));
          seg.update(p.second, make_pair(1, p.second));
        }
      } else {
        pair<int, int> p = segB.query(0, M);
        if (p.first == 0) {
          segB.update(p.second, make_pair(1, p.second));
          seg.update(p.second, make_pair(1, p.second));
        }
      }
      pair<int, int> p = seg.query(0, M);
      if (s[p.second] == 'A') cout << "Alice" << endl;
      else cout << "Bob" << endl;
    }
  }
  return 0;
