...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> s >> q) {
    using S = int;
    using F = int;
    LazySegTree<S, F>::op_fn op = [] (S a, S b) { return min(a, b); };
    LazySegTree<S, F>::mapping_fn mapping = [] (F f, S x) { x += f; return x; };
    LazySegTree<S, F>::composition_fn composition = [] (F a, F b) { return a + b; };
    const int inf = (1 << 29);
    LazySegTree<S, F> seg(s.size(), op, mapping, composition, inf, 0);
    ...
    while (q--) {
      ...
      if (op == 2) {
        ...
        cin >> begin >> end;
        --begin;
        int x = seg.query(begin, end);
        if (begin) x -= seg.get(begin - 1);
