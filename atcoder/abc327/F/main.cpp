...
{
  int n, interval, width;
  while (cin >> n >> interval >> width) {
    vec<pair<int, int>> v(n); // (time, position)
    cin >> v;
    sort(v.begin(), v.end());
    ...
    auto fn = [&] (pair<int, int> p, int add) {
      auto [t, x] = p;
      int begin = max(0, x - width);
      int end   = min(N - 1, x + 1);
