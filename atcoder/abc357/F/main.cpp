...
int main(int argc, char *argv[])
{
  ...
  lazy_segtree::mapping_fn mapping = [] (F a, S b) {
    b.Z += b.B * a.first;
    b.Z += b.A * a.second;
    b.Z += a.first * a.second * b.len;
    b.A += b.len * a.first;
    b.B += b.len * a.second;
    return b;
  };
  lazy_segtree::composition_fn composition = [] (F a, F b) {
    a.first += b.first;
    a.second += b.second;
    return a;
  };
  ...
  while (cin >> n >> q) {
    ...
    S e{mint(0), mint(0), mint(0), mint(0)};
    pair<mint, mint> id = make_pair(mint(0), mint(0));
    lazy_segtree seg(n, op, mapping, composition, e, id);
    for (int i = 0; i < n; ++i) {
      S s {a[i] * b[i], a[i], b[i], mint(1)};
      seg.set(i, s);
    }
    ...
    while (q--) {
      ...
      if (op == 1) {
        cin >> x;
        seg.apply(begin, end, make_pair(x, mint(0)));
      }
      if (op == 2) {
        cin >> x;
        seg.apply(begin, end, make_pair(mint(0), x));
      }
      if (op == 3) {
