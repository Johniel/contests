...
lli fn(const str& s)
{
  SegTree<lli> seg(s.size(), 0, [] (auto x, auto y) { return x + y; });
...

  {
    ...
    lli x = count(a.begin(), a.end(), '0') * 2 + count(a.begin(), a.end(), '1');
    setmin(mn, x);
  }
  {
    str a = s;
  ...
  for (int i = 0; i + 1 < v.size(); ++i) {
    if (i + 1 < v.size()) {
      const int j = i + 1;
      const int suff = s.size() - v[j];
      const int pref = v[i] + 1;

      int p1 = seg.query(0, v[i] + 1);
      int p0 = pref - p1;
      int s1 = seg.query(v[j], s.size());
      int s0 = suff - s1;

      lli cost = 0;
      cost += (p1 + p0 * 2) + (s1 + s0 * 2);
