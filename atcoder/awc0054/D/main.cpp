...
const int N = 3000 + 3;
pair<lli, pair<lli, lli>> memo[N][N];
pair<lli, pair<lli, lli>> rec(int i, int j)
{
  if (i == j) return make_pair(a[i], make_pair(a[i], 0));
  if (memo[i][j].first != -inf) return memo[i][j];

  pair<lli, pair<lli, lli>> x = rec(i + 1, j);
  x.first *= -1;
  x.first += a[i];
  swap(x.second.first, x.second.second);
  x.second.first += a[i];

  pair<lli, pair<lli, lli>> y = rec(i, j - 1);
  y.first *= -1;
  y.first += a[j];
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    a.resize(n);
    cin >> a;
    fill(&memo[0][0], &memo[N - 1][N - 1] + 1, make_pair(-inf, make_pair(0, 0)));
    auto p = rec(0, a.size() - 1);
