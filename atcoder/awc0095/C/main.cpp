...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    dp[0] = 1;
    for (int i = 0; i < n; ++i) {
      unless (binary_search(b.begin(), b.end(), i + 1)) (dp[i + 1] += dp[i]) %= mod;
      unless (binary_search(b.begin(), b.end(), i + 2)) (dp[i + 2] += dp[i]) %= mod;
      unless (binary_search(b.begin(), b.end(), i + 3)) (dp[i + 3] += dp[i]) %= mod;
    }
    cout << dp[n] << endl;
  }
  return 0;
