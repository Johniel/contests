...
// atcoder/typical90/017/main.cpp
...
private:
  ...
  inline bool is_leaf(size_t k) const { return bitceiln <= k; }
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    LazySegTree<lli, lli>::op_fn op = [] (lli x, lli y) { return x + y; };
    LazySegTree<lli, lli>::mapping_fn mapping = [] (lli x, lli y) { return x + y; };
    LazySegTree<lli, lli>::composition_fn composition = [] (lli x, lli y) { return x + y; };

    LazySegTree<lli, lli> seg(N, op, mapping, composition, 0, 0);
    ...
    each (k, v) {
      z += seg.get(k.first);
      seg.apply(k.first + 1, k.second - 1, 1);
    }
    cout << z << endl;
  }
  return 0;
