...
int main(int argc, char *argv[])
{
  ...
  // dist(P1,P2)=max(x1−x2+y1−y2,x1−x2+y2−y1,x2−x1+y1−y2,x2−x1+y2−y1)
  ...
  while (cin >> n >> q) {
    ...
    SegTree<lli> seg1(v.size(), -inf, [] (auto a, auto b) { return max(a, b); });
    SegTree<lli> seg2(v.size(), -inf, [] (auto a, auto b) { return max(a, b); });
    SegTree<lli> seg3(v.size(), -inf, [] (auto a, auto b) { return max(a, b); });
    SegTree<lli> seg4(v.size(), -inf, [] (auto a, auto b) { return max(a, b); });
    ...
    while (q--) {
      ...
      if (op == 2) {
        ...
        --begin;
        vec<lli> v;
        v.push_back(seg1.query(begin, end) - X - Y);
        v.push_back(seg2.query(begin, end) - X + Y);
        v.push_back(seg3.query(begin, end) + X - Y);
        v.push_back(seg4.query(begin, end) + X + Y);
        // cout << v << endl;
