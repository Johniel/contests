...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> q) {
    ...
    map<int, int> rev;
    {
      ...
      a.push_back(0);
      a.push_back(n);
      each (i, v) a.push_back(i.first), a.push_back(i.second);
      sort(a.begin(), a.end());
    ...
    vec<lli> a(rename.size(), 0);
    for (int i = 0; i + 1 < rename.size(); ++i) {
      int x = rev[i];
      int y = rev[i + 1];
      a[i] = y - x;
    }
    ...
    // for (int i = 0; i < rename.size(); ++i) cout << beats.range_sum(i, i +1) << ","; cout << endl;
    each (k, v) {
      auto [begin, end] = k;
      begin = rename[begin];
      end = rename[end];
      beats.range_chmin(begin, end, 0);
      // for (int i = 0; i < rename.size(); ++i) cout << beats.range_sum(i, i +1) << ","; cout << endl;
      cout << beats.range_sum(0, rename.size()) << endl;
    }
  }
  return 0;
}
