...
lli rec(int begin, int end, int i, SegTree<pair<int, int>>& seg)
{
  lli z = 0;
  {
    auto [_, j] = seg.query(begin, i);
    if (0 <= _) setmax(z, rec(begin, i, j, seg) + abs(i - j));
  }
  {
    auto [_, j] = seg.query(i + 1, end);
    if (0 <= _) setmax(z, rec(i + 1, end, j, seg) + abs(i - j));
  }
  return z;
}
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    const int inf = (1 << 28);
    SegTree<pair<int, int>> seg(n, make_pair(-inf, -1), [] (auto a, auto b) { return max(a, b); });
    for (int i = 0; i < n; ++i) {
      seg.set(i, make_pair(p[i], i));
    }
    cout << rec(0, n, inv[n - 1], seg) << endl;
  }
  return 0;
}
