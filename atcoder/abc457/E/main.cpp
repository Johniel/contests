...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    cin >> v;
    map<lli, vec<lli>> head, tail;
    each (k, v) {
      head[k.first].push_back(k.second);
      tail[k.second].push_back(k.first);
    }
    each (k, head) sort(k.second.begin(), k.second.end());
    each (k, tail) sort(k.second.begin(), k.second.end());
    ...
    SegTree<int> seg(N, inf, [] (auto a, auto b) { return min(a, b); });
    each (k, v) {
      int x = seg.get(k.first);
      seg.set(k.first, min(x, k.second));
    }
    ...
    while (q--) {
      ...
      { // ２枚使う
        ...
        x = y = make_pair(0, 0);
        if (head.count(s)) {
          auto itr = lower_bound(head[s].begin(), head[s].end(), t + 1);
          if (itr != head[s].begin()) {
            --itr;
            if (*itr <= t) x = make_pair(s, *itr);
          }
        }
        if (tail.count(t)) {
          auto itr = lower_bound(tail[t].begin(), tail[t].end(), s + 1);
          if (itr != tail[t].end() && s <= *itr) {
            y = make_pair(*itr, t);
          }
        }
        if (x.first == s && y.second == t && x.second <= t && s <= y.first && y.first <= x.second + 1) {
          f = true;
        }
      }
      if (!f) { // S側１枚+覆われてる小さな１枚
        auto x = make_pair(s, t);
        if (vis.count(x) && 2 <= vis[x]) f = true;
        if (vis.count(x) && vis[x] == 1) {
          if (seg.query(s + 1, t + 1) <= t) f = true;
          if (seg.query(s, t) < t) f = true;
        }
      }
      cout << (f ? "Yes" : "No") << endl;
    }
