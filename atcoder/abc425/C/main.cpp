...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> q) {
    vec<lli> a(n);
    cin >> a;
    SegTree<lli> seg(a, 0, [] (lli x, lli y) { return x + y; });
    int head = 0;
    while (q--) {
      int op;
      cin >> op;
      ...
      if (op == 2) {
        ...
        (end += head) %= n;
        ++end;
        // cout << make_pair(begin, end) << ' ' << head << endl;
        if (begin < end) {
          lli x = seg.query(begin, end);
          cout << x << endl;
        } else {
          lli x = seg.query(begin, n);
          lli y = seg.query(0, end);
          cout << x + y << endl;
        }
      }
    }
