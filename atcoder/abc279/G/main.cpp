...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k >> _c) {
    const mint c(_c);
    const int N = 1e6 + 3;
    SegTree<mint> seg1(N, mint(0), [] (auto a, auto b) { return a + b; });
    SegTree<mint> seg2(N, mint(0), [] (auto a, auto b) { return a + b; });
    for (int i = 0; i <= n; ++i) {
      // 単色 + 別の色に切り替え + 片方の色を残す
      int a = max(0, i - k + 2);
      mint x = c + seg1(0, a) * (c - 1) + seg2(0, a);
      // 別の色を加えて2色 + これまでの2色を継続
      int b = max(0, i - (k - 1) + 1);
      int d = max(0, i);
      mint y = seg1(b, d) * (c - 1) + seg2(b, d);
      seg1.update(i, x);
      seg2.update(i, y);
    }
    cout << seg1[n-1] + seg2[n-1] << endl;
  }
  return 0;
}
