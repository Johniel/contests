...
int main(int argc, char *argv[])
{
  ...
  while (cin >> q >> k) {
    ...
    const int N = 2 * 1e5 + 3;
    SegTree<int> seg(m.size(), 0, [] (auto x, auto y) { return x + y; });
    ...
    each (_, v) {
      // cout << "S:" << S << endl;
      // cout << "T:" << T << endl;
      // for (int i = 0; i < m.size(); ++i) cout << seg.get(i) << ' '; cout << endl;
      ...
      if (op == 1) {
        if (S.count(x)) {
          ...
          T.erase(x);
          seg.set(m[x], 0);
          auto itrSend = S.lower_bound(x);
          auto itrSbegin = S.lower_bound(x);
          if (itrSend != S.end() && itrSbegin != S.begin()) {
            --itrSbegin;
            if (k < abs(*itrSbegin - *itrSend)) {
              T.insert(*itrSbegin);
            }
          }
        } else {
          auto itrTbegin = T.lower_bound(x);
          if (itrTbegin != T.begin()) {
            --itrTbegin;
            auto itrSend = S.lower_bound(x);
            assert(k < abs(*itrTbegin - *itrSend));
            if (abs(*itrTbegin - x) <= k) {
              T.erase(*itrTbegin);
            }
            if (k < abs(*itrSend - x)) {
              T.insert(x);
      ...
      if (op == 2) {
        ...
        cout << seg.query(begin, min<int>(seg.size(), end + 1)) << endl;
      }
    }
  }
  return 0;
