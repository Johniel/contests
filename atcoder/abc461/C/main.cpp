...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k >> m) {
    ...
    each (k, x) sort(k.second.begin(), k.second.end());
    vec<int> a;
    each (k, x) a.push_back(k.first);
    sort(a.begin(), a.end(), [&] (auto i, auto j) {
      return x[i].back() < x[j].back();
    });
    lli z = 0;
    while (m--) {
      // cout << x << endl;
      // cout << a << endl;
      assert(x.count(a.back()));
      assert(x[a.back()].size());
      z += x[a.back()].back();
      x[a.back()].pop_back();
      a.pop_back();
      --k;
