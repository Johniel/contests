...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    map<int, int> rename;
    {
      each (i, v) sort(i.begin(), i.end());
      sort(v.begin(), v.end());
      vec<int> u;
      each (i, v) copy(i.begin(), i.end(), back_inserter(u));
      sort(u.begin(), u.end());
      u.erase(unique(u.begin(), u.end()), u.end());
      for (int i = 0; i < u.size(); ++i) {
        rename[u[i]] = i;
      }
      each (i, v) each (j, i) j = rename[j];
      each (i, v) i[1] *= -1, i[2] *= -1;
      sort(v.begin(), v.end());
      each (i, v) i[1] *= -1, i[2] *= -1;
    }
    ...
    const int N = 2 * 1e5;
    const int M = N * 3 + 3;
    SegTree<int> seg(M, 1e9+3, [] (auto a, auto b) { return min(a, b); });

    // {h, w, d}
    bool f = false;
    each (a, v) {
      f = (f || seg(0, a[1]) < a[2]);
      seg.update(a[1], min(a[2], seg[a[1]]));
    }
    cout << (f ? "Yes" : "No") << endl;
  }
  return 0;
