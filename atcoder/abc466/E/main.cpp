...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    fill(&dp[0][0][0], &dp[N - 1][K - 1][2 - 1] + 1, -inf);
    dp[0][0][false] = 0;
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j <= k; ++j) {
        setmax(dp[i + 1][j][false], dp[i][j][false] + v[i].first);
        setmax(dp[i + 1][j][true], dp[i][j][true] + v[i].second);
        if (j + 1 <= k) {
          setmax(dp[i + 1][j + 1][true], dp[i][j][false] + v[i].second);
        }
        setmax(dp[i + 1][j][false], dp[i][j][true] + v[i].first);
      }
    }
    lli z = -inf;
    for (int j = 0; j <= k; ++j) {
      setmax(z, dp[n][j][false]);
      setmax(z, dp[n][j][true]);
    }
    cout << z << endl;
  }
