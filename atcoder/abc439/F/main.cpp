...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    SegTree<lli> seg1(a.size(), 0LL, [] (auto a, auto b) { return (a + b) % mod; });
    SegTree<lli> seg2(a.size(), 0LL, [] (auto a, auto b) { return (a + b) % mod; });
    SegTree<lli> seg3(a.size(), 0LL, [] (auto a, auto b) { return (a + b) % mod; });

    for (int i = 0; i < a.size(); ++i) {
      { // up
        lli x = 0;
        (x += seg1.query(0, a[i])) %= mod; // 更に上がる
        (x += seg2.query(0, a[i])) %= mod; // 下がって上がる
        (x += seg3.query(0, a[i])) %= mod; // 初めて上がる
        seg1.set(a[i], x);
      }
      { // down
        lli x = 0;
        (x += seg1.query(a[i] + 1, n)) %= mod; // 更に下がる
        (x += seg2.query(a[i] + 1, n)) %= mod; // 上がって下がる
        seg2.set(a[i], x);
      }
      seg3.set(a[i], 1);
    }
