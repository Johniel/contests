...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    cin >> v;
    v.insert(v.begin(), makepair(0, 0));
    const int A = 2 * 1e5 + 1;
    SegTree<int> seg(A, +(1 << 29), [] (auto i, auto j) { return min(i, j); });
    seg.set(0, 0);
    ...
    for (int i = 1; i < v.size(); ++i) {
      const auto [c, _] = v[i];
      auto pred = [&] (int mn) { return max(0, i - c) <= mn; };
      int small = 0;
      int large = seg.size();
      while (small + 1 < large) {
        int mid = (small + large) / 2;
        if (pred(seg(0, mid))) small = mid;
        else large = mid;
      }
      int mex = small;
      seg.set(mex, i);
