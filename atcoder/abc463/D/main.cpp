...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    auto fn = [&] (const lli gap) {
      LazySegTree<lli, lli>::op_fn op = [] (lli a, lli b) { return max(a, b); };
      LazySegTree<lli, lli>::mapping_fn mapping = [] (lli f, lli x) { return max(x, f); };
      LazySegTree<lli, lli>::composition_fn composition = [] (lli a, lli b) { return max(a, b); };
      const int N = 2 * 1e5 + 3;
      const lli inf = (1LL << 50);
      LazySegTree<lli, lli> seg(N, op, mapping, composition, -inf, 0);
      seg.set(0, 0);

      for (int i = 0; i < v.size(); ++i) {
        const auto x = seg.get(i);
        auto y = seg.get(i + 1);
        {
          seg.set(i + 1, max(x, y));
        }
        {
          auto itr = lower_bound(v.begin(), v.end(), make_pair(v[i].second + gap + 1, -1LL));
          const int j = itr - v.begin();
          seg.set(j, max(y, x + 1));
        }
      }
      return k <= seg.all_prod();
    };
    ...
    if (0) ;
    else if (fn(large)) cout << large+1 << endl;
    else if (fn(small)) cout << small+1 << endl;
    else cout << -1 << endl;
  }
  return 0;
}
