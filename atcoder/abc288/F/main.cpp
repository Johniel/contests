...
template<typename T> istream& operator >> (istream& is, SegTree<T>& seg) {
...
template<typename T> ostream& operator << (ostream& os, SegTree<T>& seg) {
  vec<T> v;
  for (int i = 0; i < seg.n; ++i) v.push_back(seg[i]);
  os << v;
  return os;
}
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> s) {
    ...
    seg.update(0, mint(v.front()));
    for (int i = 1; i < v.size(); ++i) {
      mint x = seg.query(0, i) * v[i];
      mint y = seg(i - 1) * 10 + v[i];
      seg.update(i, x + y);
    }
    cout << seg(n-1) << endl;
  }
