...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    SegTree<lli> seg(n, 1LL << 60, [] (auto a, auto b) { return min(a, b); });
    for (int i = 0; i < n; ++i) {
      seg.update(i, c[i]);
    }

    static lli dp[N][N]; // dp[i番目まで見た][i以下で残っているのはj個] := 最小
    const lli inf = 1LL << 60;
    fill(&dp[0][0], &dp[N - 1][N - 1] + 1, inf);
    dp[0][0] = 0;
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        bool f = binary_search(x.begin(), x.end(), i);
        { // 買う
          setmin(dp[i + 1][j + 1], dp[i][j] + a[i] + seg(i - j, i+1));
        }
        if (!f) {
