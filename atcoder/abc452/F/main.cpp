    ...
    auto fn = [&] (const lli k) { // awcありがとう
      ...
      each (i, a) {
        ...
        while (k < x) {
          ...
          // if (q.empty()) cout << k << ' ' << x <<' ' << q << endl;
          assert(q.size());
          // x -= bit.query(q.front(), N);
          x -= bit.query(0, q.front());
          bit.add(q.front(), -1);
          q.pop();
        }
        z += q.size();
