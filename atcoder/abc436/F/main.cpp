...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    int mn = inv[0];
    int mx = inv[0];
    SegTree<lli> seg(n, 0, [] (auto x, auto y) { return x +y; });
    int k = 0;
    for (int b = 0; b < n; ++b) {
      setmin(mn, inv[b]);
      setmax(mx, inv[b]);

      lli x = seg.query(0, inv[b] + 1)+ 1;
      z += x * ((b + 1) - x + 1);

      seg.set(inv[b], 1);
      // if (seg.query(mn, mx + 1) != (mx + 1) - mn) {
      //   // cout << 1 << "," << b+1 << ' ' << make_pair(mn, mx) << seg.query(mn, mx + 1) << endl;
      //   ++z;
      // }
    }
