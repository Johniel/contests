...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    LazySegTree<lli, lli>::op_fn op = [] (auto x, auto y) { return x + y; };
    LazySegTree<lli, lli>::mapping_fn mapping = [] (auto x, auto y) { return x + y; };
    LazySegTree<lli, lli>::composition_fn composition = [] (auto x, auto y) { return x + y; };
    LazySegTree<lli, lli> seg(a.size(), op, mapping, composition, 0, 0);
    for (int i = 0; i < a.size(); ++i) {
      seg.set(i, a[i]);
    }

    for (int i = 0; i < a.size(); ++i) {
      int x = min<lli>(n - i - 1, seg.get(i));
      // cout << i << ' ' << x << endl;
