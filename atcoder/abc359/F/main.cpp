...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    SegTree<pair<lli, int>> seg(a.size(), make_pair(1LL << 60, -1), [] (auto x, auto y) {return min(x, y); });
    seg.set(0, make_pair(a[0], 0));
    vec<lli> x(n, 0);

    lli z = 0;
    for (int i = 1; i < a.size(); ++i) {
      pair<lli, int> p = seg.query(0, i);
      ...
      seg.set(i, make_pair(a[i] * (x[i] * 2 + 1), i));
      seg.set(j, make_pair(a[j] * (x[j] * 2 + 1), j));
    }
