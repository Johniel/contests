...
int main(int argc, char *argv[])
{
  ...
  while (cin >> s) {
    ...
    each (c, s) ++m[c];
    vec<pair<int, char>> v;
    each (k, m) v.push_back(make_pair(k.second, k.first));
    sort(v.begin(), v.end());
    reverse(v.begin(), v.end());
    vec<int> u;
    for (int i = 0; i < s.size(); i += 2) u.push_back(i);
    for (int i = 1; i < s.size(); i += 2) u.push_back(i);
    // cout << u << endl;
    str t(s.size(), '_');
    {
      int i = 0;
      each (k, v) {
        for (int _ = 0; _ < k.first; ++_) {
          t[u.at(i++)] = k.second;
        }
      }
    }
    // cout << t << endl;
    bool f = (count(t.begin(), t.end(), '_') == 0);
    for (int i = 0; i + 1 < t.size(); ++i) {
      f = f && (t[i] != t[i + 1]);
    }

    if (f) cout << "Yes" << endl << t << endl;
    else cout << "No" << endl;
  }
  return 0;
}
