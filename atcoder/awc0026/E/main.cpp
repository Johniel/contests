...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    SegTree<lli> mx(a, -inf, [] (auto x, auto y) { return max(x, y); });
    SegTree<lli> mn(a, +inf, [] (auto x, auto y) { return min(x, y); });
    auto fn = [&] (lli k) {
      lli z = 0;
      for (int i = 0; i < a.size(); ++i) {
        ...
        while (small + 1 < large) {
          ...
          if (mx(i, mid) - mn(i, mid) <= k) small = mid;
          else large = mid;
        }

        if (0) ;
        else if (mx(i, large) - mn(i, large) <= k) z += large - i;
        else if (mx(i, small) - mn(i, small) <= k) z += small - i;
      }
      return z;
    };
    // cout << fn(k + 1) - fn(k) << endl;
    cout << fn(k) - fn(k - 1) << endl;
  }
  return 0;
}
