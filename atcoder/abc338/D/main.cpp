...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    for (int i = 0; i + 1 < v.size(); ++i) {
      ...
      // cout << i << "th " << make_pair(src, dst) << ' ' << make_pair(x, y) << endl;
      const lli diff = abs(x - y);
      if (x < y) {
        seg.apply(src, dst, diff);
      } else {
        seg.apply(dst, n, diff);
        seg.apply(0, src, diff);
      }
    }
    // if (n < 1000) for (int i = 0; i < n; ++i) cout << seg.get(i) << endl;
    // cout << "sum:" << sum << endl;
    lli z = seg.all_prod();
    cout << sum + z << endl;
    // cout << endl;
