...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    // aでのindex, bでのindex
    SegTree<pair<int, int>> mn(n, make_pair(1 << 29, -1), [] (pair<int, int> a, pair<int, int> b) { return min(a, b); });
    for (int i = 0; i < b.size(); ++i) {
      mn.update(i, make_pair(m[b[i]], i));
    }

    vec<pair<int, int>> v(n, {-1, -1});
    function<int(int, int)> rec = [&] (int begin, int end) {
      if (begin == end) return -1;
      pair<int, int> p = mn(begin, end);
      int i = p.second;
      v[b[i]].first = rec(begin, i);
      v[b[i]].second = rec(i + 1, end);
      return b[p.second];
    };
    rec(0, n);

    vec<int> c, d;
    pair<int, int> p = mn(0, n);
    function<void(int)> fn = [&] (int curr) {
      if (curr == -1) return ;
      c.push_back(curr);
      fn(v[curr].first);
      d.push_back(curr);
