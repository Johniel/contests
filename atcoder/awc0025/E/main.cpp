...
struct Splay {
  ...
  struct Node {
    ...
    T value;
    size_t sz;
    Node(T v) : parent(nullptr), right(nullptr), left(nullptr), value(v), sz(1) {}
  ...
  bool erase_by_order(int nth) {
    auto itr = find_by_order(nth);
    if (itr == end()) return false;
    return erase_by_value(*itr);
  }
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> q) {
    ...
    Splay<pair<int, int>> s;
    for (int i = 0; i < d.size(); ++i) {
      assert(s.insert(make_pair(d[i], i), i));
    }
    ...
    while (q--) {
      ...
      --t;
      if (t < s.size()) {
        pair<int, int> p = *s.find_by_order(t);
        assert(s.erase_by_order(t));
        --p.first;
        if (p.first) s.insert(p, t);
      }
      cout << s.size() << endl;
    }
  }
