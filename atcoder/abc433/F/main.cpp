...
int main(int argc, char *argv[])
{
  ...
  while (cin >> s) {
    ...
    PrefixSum<lli> acc[M];
    for (int i = 0; i < M; ++i) {
      acc[i] = PrefixSum<lli>(v[i]);
    }
    lli z = 0;
    for (int i = 0; i < s.size(); ++i) {
      int x = s[i] - '0';
      if (x == 9) continue;
      int pref = acc[x](0, i + 1) - 1;
      int suff = acc[x + 1](i + 1, N);
      int mn = min(pref, suff);
      (z += math::vandermonde_convolution(pref, suff, pref + 1) % mod) %= mod;
    }
    cout << z << endl;
  }
  return 0;
