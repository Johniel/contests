...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> k) {
    ...
    lli z = 0;
    for (int i = 0; i < a.size(); ++i) {
      const lli s = (sum - a[i]) % mod;

      const lli p = math::nCk(n - 1, k - 1);
      const lli q = math::nCk(n - 2, k - 2);
      lli x = s * p % mod;

      lli y = (s * q % mod) + ((p * a[i]) % mod);
      (z += y * a[i] % mod) %= mod;

      // cout << a[i] << "*" << y <<"=" << a[i]* y << ", " << p << ", s=" << s << endl;
    }
    cout << z << endl;
  }
  return 0;
