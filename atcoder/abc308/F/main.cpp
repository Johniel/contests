...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    // cout << "p:" << p << endl; cout << "v:" << v << endl;

    SegTree<pair<lli, int>> seg(p.size(), make_pair(1LL << 60, -1), [] (pair<lli, int> a, pair<lli, int> b) {
    ...
    each (i, v) {
      auto itr = lower_bound(p.begin(), p.end(), i.first);
      if (itr == p.end()) continue;
      int begin = itr - p.begin();
      pair<lli, int> p = seg.query(begin, n);
      // cout << i << ' ' << p << endl;
      if (p.first < i.second) {
        seg.update(p.second, make_pair(i.second, p.second));
      }
    }
