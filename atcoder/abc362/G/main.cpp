...
int main(int argc, char *argv[])
{
  ...
  while (cin >> s >> q) {
    ...
    vec<int> idx = atcoder::suffix_array(s);
    vec<int> lcp = atcoder::lcp_array(s, idx);

    SegTree<int> seg(s.size(), 1 << 29, [] (auto a, auto b) { return min(a, b); });
    ...
    while (q--) {
      ...
      {
        auto fn = [&] (int i) {
          return 0 <= strcmp(t.c_str(), s.c_str() + idx[i]);
        };
        int small = 0;
        int large = s.size();
        while (small + 1 < large) {
      ...
      }
      {
        ...
        while (small + 1 < large) {
          int mid = (small + large)/  2;
          if (t.size() <= seg.query(x, mid)) small = mid;
          else large = mid;
        }
        cout << large - x << endl;
      }
