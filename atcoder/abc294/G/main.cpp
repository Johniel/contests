...
void rec(int curr, int prev)
{
  ...
  each (e, g[curr]) {
    ...
    vis[make_pair(curr, next)] = make_pair(cost.size(), -1);
    vis[make_pair(next, curr)] = make_pair(cost.size(), -1);
    cost.push_back(+e.second);

    rec(next, curr);

    vis[make_pair(curr, next)].second = cost.size();
    vis[make_pair(next, curr)].second = cost.size();
    cost.push_back(-e.second);
  }
  post[curr] = ord++;
  return ;
...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n) {
    ...
    fill(pre, pre + N, -(1LL << 60));
    fill(post, post + N, -(1LL << 60));
    cost.clear();
    ord = 0;
    vis.clear();
    rec(0, 0);

    SegTree<lli> sum(cost.size(), 0, [] (auto x, auto y) { return x + y; });
    ...
    while (q--) {
      ...
      if (op == 1) {
        ...
        --i;
        pair<int, int> e = es[i];
        sum.update(vis[e].first,  +x);
        sum.update(vis[e].second, -x);
      }
      if (op == 2) {
        int i, j;
        cin >> i >> j;
        ...
        --j;
        int k = LCA::query(i, j);
        lli z = sum.query(0, pre[k]);
        lli x = sum.query(0, pre[i]);
        lli y = sum.query(0, pre[j]);
        cout << x + y - 2 * z << endl;
      }
    }
  }
