...
int main(int argc, char *argv[])
{
  ...
  while (cin >> n >> m) {
    ...
    for (int i = 0; i < n; ++i) {
      q.push(make_pair(-a[i][idx[i]], make_pair(i, idx[i])));
    }
    ...
    while (q.size()) {
      ...
      lli x = bit(i+1, n) + (n - (i + 1)) * j;
      // cout << p << ", +" << x << endl;
      z += x;
      bit.add(i, 1);
      if (j + 1 < a[i].size()) {
        q.push(make_pair(-a[i][j + 1], make_pair(i, j + 1)));
      }
    }
